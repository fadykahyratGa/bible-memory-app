...
2. **تحديث القيم في الكود**
   ...
   - (اختياري موصى به) أضف عمود `api_index` في جدول `books` لتطابق ترقيم خدمة `arabic-bible.onrender.com`، مما يسمح بالتحقق من النصوص مباشرة من واجهة البرمجة.
3. **تشغيل التطبيق**
   - تأكد من تثبيت Flutter 3/Dart 3.
   - شغّل: `flutter pub get` ثم `flutter run`.

> **ملاحظة حول التحقق من النص**: يستخدم التطبيق خدمة `https://arabic-bible.onrender.com/` تلقائيًا للحصول على قائمة الأسفار القانونية (مع تجاهل الأسفار القانونية الثانية) وللتحقق من نص الآيات عند تشغيل اللعبة، مما يضمن مطابقة النصوص العربية الرسمية.
